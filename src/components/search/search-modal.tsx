...
import { searchPages, type SearchResult } from '@/lib/actions/pages'
...
export function SearchModal() {
  ...
  const navigate = useCallback((result: SearchResult) => {
    setOpen(false)
    if (result.dailyDate) {
      router.push(`/daily?date=${result.dailyDate}`)
    } else {
      router.push('/daily')
    }
  }, [router])
  ...
  const highlightMatch = (text: string, q: string) => {
    ...
    return (
      <>
        ...
        <strong style={{ color: '#2563eb', fontWeight: 600 }}>{snippet.slice(matchStart, matchStart + q.length)}</strong>
            ...
            {results.map((r, i) => (
              <button
                ...
                style={{
                  ...
                  border: 'none',
                  cursor: 'pointer',
                  backgroundColor: i === selectedIndex ? '#f3f4f6' : 'transparent',
