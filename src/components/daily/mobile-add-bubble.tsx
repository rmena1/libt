...
export function MobileAddBubble({ isOpen, onClose, onPageCreated }: MobileAddBubbleProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isTask) {
        ...
        if (priority) pageContent += ` !${priority}`
        if (taskDate && taskDate !== dailyDate) pageContent += ` @${taskDate}`
      }
  ...
  const backdropStyle: React.CSSProperties = {
    ...
    zIndex: 9998, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', padding: '16px',
  }
  
  const bubbleStyle: React.CSSProperties = {
    backgroundColor: '#ffffff', borderRadius: '16px', padding: '20px',
    width: '100%', maxWidth: '400px', boxShadow: '0 10px 40px rgba(0, 0, 0, 0.2)', marginBottom: '60px',
  }
  
  const inputStyle: React.CSSProperties = {
    width: '100%', padding: '12px', fontSize: '16px', border: '1px solid #e5e7eb',
    borderRadius: '8px', outline: 'none', resize: 'none', fontFamily: 'inherit', minHeight: '80px',
  }
  ...
  const getPriorityPillStyle = (p: 'low' | 'medium' | 'high'): React.CSSProperties => {
    ...
    return {
      padding: '6px 14px', fontSize: '13px', fontWeight: 500, borderRadius: '999px',
      border: `1px solid ${colors[p].border}`, backgroundColor: colors[p].bg,
      color: colors[p].color, cursor: 'pointer', transition: 'all 0.15s ease',
    }
  }
      ...
      <div style={{ ...bubbleStyle, position: 'relative' }}>
        <button
          style={{
            ...
            borderRadius: '50%', border: 'none', backgroundColor: '#f3f4f6', color: '#6b7280',
        ...
        <textarea
          ...
          onKeyDown={handleKeyDown} placeholder="What's on your mind?" style={inputStyle}
        />
        
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginTop: '16px', paddingTop: '16px', borderTop: '1px solid #f3f4f6' }}>
        ...
        {isTask && (
          <>
            <input
              ...
              min={today()}
              style={{ padding: '8px 12px', fontSize: '14px', border: '1px solid #e5e7eb', borderRadius: '8px', outline: 'none', width: '100%', marginTop: '12px' }}
              placeholder="Due date (optional)"
            />
            <div style={{ display: 'flex', gap: '8px', marginTop: '12px' }}>
              {(['low', 'medium', 'high'] as const).map(p => (
                <button key={p} type="button" style={getPriorityPillStyle(p)}
...
interface MobileFABProps { onClick: () => void }
...
export function MobileFAB({ onClick }: MobileFABProps) {
  ...
  return (
    <button
      ...
      onClick={onClick} aria-label="Add new note or task"
