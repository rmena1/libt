...
export function FolderListView({ initialTree }: FolderListViewProps) {
  ...
  const handleRename = async (folderId: string) => {
    ...
    try {
      await renameFolder(folderId, editName.trim())
      setEditingId(null)
    ...
    <>
      <div
        style={{
          ...
          borderBottom: isLast && !hasChildren ? 'none' : '1px solid rgba(0, 0, 0, 0.04)',
          gap: '10px',
        }}
      >
        ...
        {isEditing ? (
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter') onRename(node.folder.id)
              if (e.key === 'Escape') onCancelEdit()
            }}
            onBlur={() => onRename(node.folder.id)}
            autoFocus
            style={{
              flex: 1,
              padding: '4px 8px',
        ...
        {!isEditing && (
          <div style={{
            ...
            <button
              onClick={() => onStartEdit(node.folder.id, node.folder.name)}
              style={{
                padding: '4px',
                background: 'none',
                border: 'none',
